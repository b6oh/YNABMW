...
<template>
  <main class="pb-24">
    ...
    <div class="flex justify-between items-center my-5">
      <div v-if="balance > 0" class="inline-flex flex-shrink-0 items-center rounded-full bg-green-50 px-1.5 py-0.5 text-md font-medium text-green-700 ring-1 ring-inset ring-green-600/20 mr-3">
        Balance: {{ envelopeBalance }}
      </div>
    </div>


    <div class="flex flex-wrap justify-between mt-6" v-if="hasBalance">
    ...
    <div v-if="filteredTransactions.length > 0" class="mt-6">
      <div class="mt-3">
        ...
        <div v-for="tr in filteredTransactions" :key="tr.id" class="flex justify-between bg-white rounded-lg px-4 py-2 my-2">
